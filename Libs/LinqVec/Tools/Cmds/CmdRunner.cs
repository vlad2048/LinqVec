...
public sealed record HotspotChangedRunEvt(
	string State,
	string Hotspot
) : IRunEvt;
...
public static class CmdRunner
{
	...
	)
	{
		...
		var curHotspot = curState.TrackHotspot(evt.WhenEvt, scheduler, d);
		var cmdEvt = curHotspot.ToCmdEvt(evt.WhenEvt, scheduler, d).MakeHot(d);

		SetCursor(curState, curHotspot, evt.SetCursor, d);
		InvokeActions(cmdEvt, curHotspot, curStateSet, curStateReset, scheduler, d);

		var runEvents = GetRunEvents(curState, curHotspot, cmdEvt);

		runEvents.Subscribe(_ => invalidate()).D(d);

		G.Cfg.RunWhen(e => e.Log.LogCmd.Evt, d, [() => evt.WhenEvt.LogD("WhenEvt")]);
		G.Cfg.RunWhen(e => e.Log.LogCmd.CurStateHotspot, d, [() => Log(curState, curHotspot)]);

		return new CmdOutput(runEvents, cmdEvt);
	}
	...
	)
	{
		...
		//void Reset() => curStateFun.V = initStateFun;
	...
	)
	{
		Obs.Merge(
				curHotspot,
				cmdEvt
					.Where(e => e is ConfirmCmdEvt)
					.Select(_ => curHotspot.V)
