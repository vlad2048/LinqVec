...
namespace LinqVec.Tools._Base.Events;
...
{
    public static IObservable<MouseMoveEvtGen<T>> WhenMouseMoveEvt<T>(this IObservable<IEvtGen<T>> src) => src.OfType<MouseMoveEvtGen<T>>();
    ...
    public static IRoVar<bool> IsKeyDown<T>(this IObservable<IEvtGen<T>> src, Keys key, IRoDispBase d) =>
        Var.Make(
            false,
            src.WhenKeyDown(key).Select(_ => true).Merge(
